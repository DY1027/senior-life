...
export default function HospitalChecklistPreview() {
  ...
  const buildCopyText = () => {
    ...
    return [
      ...
      "",
      "───────────────────────────────",
      "※ 이 체크리스트는 병원 방문 준비를 돕는 참고용 도구입니다.",
      "   정확한 진단과 치료는 의료진의 안내를 따라야 합니다.",
      "═══════════════════════════════",
    ].join("\n");
  };
  ...
  const handlePrint = () => {
    ...
    const data = checklistData[selected];
    const label = visitTypes.find((v) => v.key === selected)?.label ?? "";
    const bringRows = data.bring.map((b, i) => `<tr><td style="padding:6px 8px;border:1px solid #E8F0FE;">${i + 1}</td><td style="padding:6px 8px;border:1px solid #E8F0FE;">${b}</td><td style="padding:6px 8px;border:1px solid #E8F0FE;text-align:center;">□</td></tr>`).join("");
    const questionRows = data.questions.map((q, i) => `<tr><td style="padding:6px 8px;border:1px solid #E8F0FE;">${i + 1}</td><td style="padding:6px 10px;border:1px solid #E8F0FE;">${q}</td></tr>`).join("");
    const memoRows = [
      ...data.bring.map((b, i) => `${i + 1}. ${b}`),
      "",
      "물어볼 질문:",
    ...
    <style>
      *{box-sizing:border-box;margin:0;padding:0;}
      ...
      .sub{font-size:12px;color:#4A5568;margin-bottom:24px;border-bottom:2px solid #1B6FC8;padding-bottom:12px;}
      ...
      th{background:#EFF6FF;color:#1B6FC8;font-size:13px;padding:8px;border:1px solid #DBEAFE;text-align:left;}
      ...
      @media print{body{padding:16px;font-size:13px;}h1{font-size:18px;}table{font-size:13px;}}
    </style></head><body>
    ...
    <div class="memo">
      <h2>📱 보호자에게 보낼 메모</h2>
      <p><strong>진료 목적:</strong> ${label}<br/><br/>
      <strong>챙겨야 할 것:</strong><br/>
      ${memoRows}</p>
    </div>
    ...
    <section id="checklist" style={{ background: "#F0F7FF", padding: "72px 20px" }}>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 28 }}>
          {visitTypes.map((v) => (
            <button key={v.key}
              ...
              style={{ height: 52, padding: "0 18px", fontSize: 15, fontWeight: 600, borderRadius: 12, border: `2px solid ${selected === v.key ? "#1B6FC8" : "#DBEAFE"}`, background: selected === v.key ? "#1B6FC8" : "#fff", color: selected === v.key ? "#fff" : "#1A1A2E", cursor: "pointer", display: "flex", alignItems: "center", gap: 7 }}>
        ...
        {selected && selectedData && (
          <div style={{ background: "#fff", borderRadius: 20, overflow: "hidden", border: "2px solid #DBEAFE", marginBottom: 16 }}>
            {/* 결과 헤더 */}
            <div style={{ background: "#0EA5E9", padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>{visitTypes.find(v => v.key === selected)?.icon} {selectedLabel} 준비 목록</p>
            ...
            <div style={{ padding: "24px 20px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }} className="cl-grid">
                    ...
                    {selectedData.bring.map((item) => (
                      <label key={item} style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", padding: "10px 12px", borderRadius: 10, background: checked[item] ? "#F0FDF4" : "#F0F7FF", minHeight: 44 }}>
                        <input type="checkbox" checked={!!checked[item]} onChange={() => toggleCheck(item)}
                          style={{ width: 20, height: 20, cursor: "pointer", accentColor: "#1B6FC8", flexShrink: 0, marginTop: 2 }} />
                ...
                {/* 의사에게 물어볼 질문 */}
                <div>
                  <p style={{ fontSize: 14, fontWeight: 700, color: "#1A1A2E", marginBottom: 12 }}>💬 의사에게 물어볼 질문</p>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div style={{ background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: 14, padding: "16px 18px", marginBottom: 16 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: "#92400E", marginBottom: 12 }}>📱 보호자에게 보낼 메모</p>
                <p style={{ fontSize: 12, color: "#78350F", marginBottom: 8, lineHeight: 1.7 }}>
                  <strong>진료 목적:</strong> {selectedLabel}
                </p>
                <p style={{ fontSize: 12, fontWeight: 700, color: "#78350F", marginBottom: 4 }}>챙겨야 할 것:</p>
                <ul style={{ margin: "0 0 10px 16px", padding: 0 }}>
                  {selectedData.bring.map((b, i) => (
                    <li key={i} style={{ fontSize: 12, color: "#78350F", lineHeight: 1.8 }}>{b}</li>
                  ))}
                </ul>
                <p style={{ fontSize: 12, fontWeight: 700, color: "#78350F", marginBottom: 4 }}>병원에서 물어볼 질문:</p>
                <ul style={{ margin: "0 0 0 16px", padding: 0 }}>
                  {selectedData.questions.map((q, i) => (
                    <li key={i} style={{ fontSize: 12, color: "#78350F", lineHeight: 1.8 }}>{q}</li>
                  ))}
                </ul>
              </div>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={handleCopy}
                  style={{ flex: 1, height: 52, fontSize: 15, fontWeight: 700, color: "#fff", background: "#0EA5E9", border: "none", borderRadius: 12, cursor: "pointer" }}>
                  📋 결과 복사하기
                </button>
                <button onClick={handlePrint}
                  style={{ flex: 1, height: 52, fontSize: 15, fontWeight: 700, color: "#0EA5E9", background: "#F0F9FF", border: "2px solid #BAE6FD", borderRadius: 12, cursor: "pointer" }}>
                  🖨️ 인쇄하기
                </button>
              </div>
            </div>
