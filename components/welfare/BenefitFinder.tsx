...
const BENEFITS: Benefit[] = [
  {
    ...
    tag: "소득 지원",
    tagColor: "#C4621A",
    amount: "독거 월 최대 349,700원 / 부부 월 최대 559,520원 (2026년 기준)",
    description: "만 65세 이상 소득 하위 70%에 해당하면 매월 기초연금을 받을 수 있습니다. 독거 어르신은 최대 349,700원, 부부가 모두 수급할 경우 각각 20% 감액되어 합산 559,520원(279,760원×2)을 받습니다.",
...
export default function BenefitFinder() {
  ...
  const SelectBtn = ({
    ...
    <button
      onClick={onClick}
      style={{ height: "auto", minHeight: 52, padding: "12px 16px", fontSize: 15, fontWeight: 600, borderRadius: 12, border: `2px solid ${active ? "#C4621A" : "#EFDFC0"}`, background: active ? "#C4621A" : "#fff", color: active ? "#fff" : "#3B3226", cursor: "pointer", textAlign: "left", lineHeight: 1.3 }}
    >
      {label}
  ...
  return (
    <div style={{ marginBottom: 40 }}>
      {/* 섹션 헤더 */}
      <div style={{ background: "linear-gradient(135deg,#C4621A,#E67E3F)", borderRadius: 20, padding: "28px 24px", marginBottom: 24, color: "#fff" }}>
      ...
      {/* 입력 패널 */}
      <div style={{ background: "#FDFBF4", borderRadius: 18, padding: "24px 20px", border: "1.5px solid #EFDFC0", marginBottom: 20 }}>
        {/* 나이 */}
        <div style={{ marginBottom: 22 }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: "#3B3226", marginBottom: 6 }}>
            <span style={{ color: "#C4621A" }}>01</span>  나이가 어떻게 되세요?
          </p>
        ...
        <div style={{ marginBottom: 22 }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: "#3B3226", marginBottom: 6 }}>
            <span style={{ color: "#C4621A" }}>02</span>  소득·재산 수준은 어느 정도인가요?
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        ...
        <div style={{ marginBottom: 22 }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: "#3B3226", marginBottom: 6 }}>
            <span style={{ color: "#C4621A" }}>03</span>  일상생활 거동 상태는 어떤가요?
          </p>
        ...
        <div style={{ marginBottom: 22 }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: "#3B3226", marginBottom: 6 }}>
            <span style={{ color: "#C4621A" }}>04</span>  가구 구성은 어떻게 되시나요?
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }} className="bf-mobility-grid">
        ...
        <button
          onClick={handleCheck}
          style={{ width: "100%", height: 58, fontSize: 17, fontWeight: 700, color: "#fff", background: canCheck ? "#C4621A" : "#9CA3AF", border: "none", borderRadius: 14, cursor: canCheck ? "pointer" : "default", letterSpacing: "-0.2px", transition: "background 0.2s" }}
        >
      ...
      {showResult && age && income && mobility && household && (
        <div id="bf-result">
          {/* 결과 헤더 */}
          <div style={{ background: matched.length > 0 ? "#FDF0E0" : "#FEF2F2", border: `1.5px solid ${matched.length > 0 ? "#FDDFC0" : "#FECACA"}`, borderRadius: 14, padding: "16px 18px", marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "#6B5E4C", lineHeight: 1.5 }}>
            ...
            {matched.length > 0 && (
              <button
                onClick={handleCopy}
                style={{ marginLeft: "auto", height: 40, padding: "0 16px", fontSize: 13, fontWeight: 700, color: "#C4621A", background: "#fff", border: "2px solid #FDDFC0", borderRadius: 10, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}
              >
                📋 목록 복사
              </button>
            )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 }}>
            {matched.map((b) => (
              <div key={b.id} style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #F3E7D3", overflow: "hidden" }}>
                <div style={{ padding: "18px 20px" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10, gap: 10 }}>
                    <div>
                      <span style={{ fontSize: 11, fontWeight: 700, color: b.tagColor, background: TAG_BG[b.tagColor] ?? "#FDF7E7", padding: "3px 8px", borderRadius: 6, marginRight: 8 }}>{b.tag}</span>
                      <p style={{ fontSize: 17, fontWeight: 800, color: "#3B3226", marginTop: 6, letterSpacing: "-0.3px" }}>{b.name}</p>
                    </div>
                    {b.href && (
                      <a href={b.href} style={{ flexShrink: 0, fontSize: 12, fontWeight: 700, color: "#C4621A", textDecoration: "none", whiteSpace: "nowrap", marginTop: 4 }}>
                        자세히 →
                      </a>
                    )}
                  </div>

                  <div style={{ background: "#FDFBF4", borderRadius: 10, padding: "10px 14px", marginBottom: 10 }}>
                    <p style={{ fontSize: 12, color: "#6B5E4C", fontWeight: 600, marginBottom: 3 }}>지원 내용</p>
                    <p style={{ fontSize: 14, fontWeight: 700, color: "#C4621A" }}>{b.amount}</p>
                  </div>

                  <p style={{ fontSize: 14, color: "#6B5E4C", lineHeight: 1.7, marginBottom: 10 }}>{b.description}</p>

                  <div style={{ borderTop: "1px solid #FDF7E7", paddingTop: 10, display: "flex", gap: 8, alignItems: "flex-start" }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#6B5E4C", flexShrink: 0, marginTop: 1 }}>신청</span>
                    <p style={{ fontSize: 13, color: "#6B5E4C", lineHeight: 1.65 }}>{b.how}</p>
                  </div>
                </div>
              </div>
            ))}
