...
### 이관 현황 — **키오스크 6종 전부 엔진 v2, 구 엔진 삭제됨**
- 카페(8) + 햄버거(6) + 민원(6) + 주차(5) + 마트(7) + **기차표 예매(7, 2026-07-15 신규)** = 임무 39종
- 기차표(든든기차)는 새 엔진 기능 없이 기존 조합만으로 구성: 편도/왕복=serviceTypes,
  행선지=카테고리, 열차 시간=상품, 좌석 등급·위치=옵션, 매수=수량, 인기 시간 매진=soldOutDecoy.
  출발지는 서울 고정(단순화) — 출발지 선택·좌석 배치도는 추후 확장
...
### 남은 단계 (명세 16장 기준)
1. 4단계 나머지: ATM(보이스피싱 경고 중심 — 가상 카드·가상 금액만, 실제 계좌·비번 입력 금지)
