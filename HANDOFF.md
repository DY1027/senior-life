...
- **보안 헤더** (`next.config.ts` headers()): CSP(허용 출처는 파일 주석에 이유와 함께 명시 —
  **새 외부 스크립트·스타일·이미지 출처를 추가하면 CSP도 같이 갱신할 것**),
  nosniff, X-Frame-Options SAMEORIGIN, Referrer-Policy(쿠팡으로 경로 안 새어나감),
  Permissions-Policy(카메라·마이크·위치·결제 차단), HSTS
- CSP script-src의 'unsafe-inline' 이유: Next 인라인 수화 스크립트. nonce 미들웨어는 추후 과제
- **JSON-LD는 반드시 `lib/jsonld.ts`의 safeJsonLd()로 직렬화** ('<' 이스케이프 — </script> 탈출 방지)
- 점검에서 안전 확인: 인쇄 창(document.write)은 내부 상수만 삽입, 쿠팡 키는 서버 전용 env,
  localStorage에 민감정보 없음, SW는 same-origin GET만 캐시, 외부 링크는 noopener
