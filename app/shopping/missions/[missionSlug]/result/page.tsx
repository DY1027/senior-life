...
import { getShoppingCompletionAdKey } from "@/lib/completion-affiliate";
...
export default async function ShoppingResultPage({ params }: Props) {
  ...
  if (!mission) notFound();
  const completionAdKey = getShoppingCompletionAdKey(mission.slug);
  return <><Header /><ShoppingResult mission={mission} completionAdKey={completionAdKey} /><Footer /></>;
}
