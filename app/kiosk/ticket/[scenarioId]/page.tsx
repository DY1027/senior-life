...
export async function generateMetadata({ params }: { params: Promise<{ scenarioId: string }> }): Promise<Metadata> {
  ...
  if (!scenario) return { title: "기차표 예매 연습", robots: { index: false, follow: false } };
  return {
...
export default async function TicketScenarioPage({ params }: { params: Promise<{ scenarioId: string }> }) {
  ...
  const completionAdKey = getKioskCompletionAdKey("ticket", scenario.id);
  return <KioskRunner catalog={ticketCatalog} scenario={scenario} completionAdKey={completionAdKey} />;
}
