...
test("홈 오늘의 연습은 표시된 실제 시나리오로 바로 연결된다", async ({ page }) => {
  ...
  const dailyCard = page.locator("section").filter({ hasText: "📅 오늘의 연습" });
  const homeText = (await dailyCard.locator("p").first().innerText()).replace(/^오늘은\s*/, "");
  const startLink = dailyCard.getByRole("link", { name: /연습 시작하기/ });
...
test("FAQ·생활안전 명칭과 홈 메타데이터가 현재 방향에 맞는다", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "어렵고 위험한 상황을 쉽게 확인해요" })).toBeVisible();
  await expect(page.getByText("🛡️ 그림으로 배우는 생활안전", { exact: true })).toBeVisible();
...
test("이번 주 세 가지 기록이 있으면 홈과 내 기록에 주간 도장이 표시된다", async ({ page }) => {
  ...
  const weeklyCard = page.locator("section").filter({ hasText: "🗓️ 이번 주 도전" });
  const hrefs = await weeklyCard.getByRole("link", { name: /도전하기/ }).evaluateAll((links) =>
  ...
  await page.reload();
  await expect(weeklyCard.getByText("🎉 주간 도장 획득!", { exact: true })).toBeVisible();
