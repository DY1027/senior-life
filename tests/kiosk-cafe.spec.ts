...
test.beforeEach(async ({ page }) => {
  await page.route("**/api/affiliate/completion-ad?key=*", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        imagePath: "/images/shopping/products/digital/usb-c-cable-white-2m.jpg",
        title: "완료 화면 추천 상품",
        description: "실제 상품 정보는 쿠팡에서 다시 확인하세요.",
        affiliateUrl: "https://link.coupang.com/a/test-completion-ad",
...
test("천천히 배우기: 따뜻한 아메리카노 포장 임무를 완수한다", async ({ page }) => {
  ...
  await expect(page.getByText(/포장하기.*눌러 주세요/)).toBeVisible();
  ...
  const actualShoppingImage = actualShoppingAd.getByRole("img", { name: "완료 화면 추천 상품 광고 이미지" });
...
test("키오스크 음성 안내는 조사 표기와 괄호를 읽지 않는다", async ({ page }) => {
  const guidance = page.getByTestId("kiosk-guidance");
  const expectNaturalGuidance = async () => {
    const text = await guidance.innerText();
    expect(text).not.toMatch(/을\(를\)|은\(는\)|이\(가\)|\([^)]*\)|→|·/);
    expect(text).not.toMatch(/\d+\s*(잔|개|장|통|건|자리)(으)?로/);
  };

  await page.goto("/kiosk/parking/parking-learn-card");
  await page.getByRole("button", { name: /연습 시작/ }).click();
  await expect(guidance).toContainText("차 번호 뒤 네 자리");
  await expectNaturalGuidance();
  for (const digit of ["1", "2", "3", "4"]) {
    await page.getByRole("button", { name: digit, exact: true }).click();
  }
  await page.getByRole("button", { name: "확인", exact: true }).click();
  await page.getByRole("button", { name: /흰색 승용차/ }).click();
  await expect(guidance).toContainText("할인 없이 정산하기 버튼을 눌러 주세요");
  await expectNaturalGuidance();

  await page.goto("/kiosk/cafe/cafe-learn-americano");
  await page.getByRole("button", { name: /연습 시작/ }).click();
  await expect(guidance).toContainText("포장하기 버튼을 눌러 주세요");
  await expectNaturalGuidance();
  await page.getByRole("button", { name: /포장하기/ }).click();
  await page.getByRole("button", { name: /아메리카노/ }).click();
  await expect(guidance).toContainText("온도는 따뜻하게 골라 주세요");
  await expectNaturalGuidance();
  await page.getByRole("button", { name: "따뜻하게" }).click();
  await expect(guidance).toContainText("수량을 두 잔으로 맞춰 주세요");
  await expectNaturalGuidance();

  await page.goto("/kiosk/civil/civil-learn-deungbon");
  await page.getByRole("button", { name: /연습 시작/ }).click();
  await page.getByRole("button", { name: /주민등록등본/ }).click();
  await expect(guidance).toContainText("뒷자리 가리기로 골라 주세요");
  await expectNaturalGuidance();
});
