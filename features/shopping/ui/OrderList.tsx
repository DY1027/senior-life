        ...
        {orders.length === 0 ? <p className="rounded-3xl border border-[#DCE6F4] bg-white p-8 text-center text-[17px] font-bold text-[#667287]">아직 완료한 가상 주문이 없어요.</p> : orders.map((order) => (
          <article key={order.id} data-testid={`order-card-${order.id}`} className="rounded-3xl border border-[#DCE6F4] bg-white p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><span className="text-[13px] font-extrabold text-[#246BDF]">{ORDER_STATE_LABELS[order.state]} · 연습 주문</span><h2 className="mt-1 text-[20px] font-black text-[#25324A]">{order.orderNumber}</h2></div>
            ...
            <Link href={`/shopping/orders/${order.id}`} className="mt-5 inline-flex min-h-12 w-full items-center justify-center rounded-xl bg-[#246BDF] px-5 font-extrabold text-white no-underline sm:w-auto">주문 상세 보기</Link>
          </article>
        ))}
      </div>
    </main>
